...
-- wotlk private: UnitGroupRolesAssigned exists natively but returns THREE
-- BOOLEANS (isTank, isHealer, isDamage) instead of a role string, so role
-- sorting and the #framesort @Healer/@Tank/@DPS selectors would silently
-- never match. LFD-assigned roles win; otherwise infer the role (pure-dps
-- class -> DAMAGER, else LibGroupTalents talent inspection, which the
-- required CompactRaidFrame backport bundles).
if wow.IsWotlkPrivate() then
    local nativeRoles = UnitGroupRolesAssigned
    local libGroupTalents = nil
    local talentRoleToWowRole = { melee = "DAMAGER", caster = "DAMAGER", healer = "HEALER", tank = "TANK" }
    ...
    wow.UnitGroupRolesAssigned = function(unit)
        ...
        if nativeRoles then
            local tank, healer, damager = nativeRoles(unit)

            if type(tank) == "string" then
                -- a foreign shim already returns the modern role string
                return tank
            end
        ...
        if class == "HUNTER" or class == "ROGUE" or class == "MAGE" or class == "WARLOCK" then
            -- these classes only ever dps in wotlk
            return "DAMAGER"
        end

        if libGroupTalents == nil then
            libGroupTalents = (LibStub and LibStub:GetLibrary("LibGroupTalents-1.0", true)) or false
        end

        local role = libGroupTalents and libGroupTalents:GetUnitRole(unit)
        return talentRoleToWowRole[role] or "NONE"
    end

    -- wotlk private EditMacro doesn't return the macro id and callers index a cache with it
    local nativeEditMacro = wow.EditMacro
    wow.EditMacro = function(id, ...)
        local newId = nativeEditMacro(id, ...)
        return newId or id
    end
end
